...
/* Le istanze statiche e non il file variabile del sito: satori non sa leggere
   la tabella `fvar` e si ferma. Le scarica `pnpm fonts:download`. */
...
export default async function OpengraphImage() {
  ...
  const loaded = typeof settings?.ogImage === 'object' ? settings.ogImage : null
  if (loaded?.filename) {
    try {
      const file = await readFile(path.join(mediaDir, loaded.filename))
      return new Response(new Uint8Array(file), {
        headers: { 'Content-Type': loaded.mimeType || 'image/jpeg' },
      })
    } catch {
  ...
  const crest = typeof settings?.logo === 'object' ? settings.logo : null
  let crestData: string | null = null
  if (crest?.filename) {
    try {
      ...
      const png = await sharp(await readFile(path.join(mediaDir, crest.filename)))
        .resize(440)
        .png()
        .toBuffer()
      crestData = `data:image/png;base64,${png.toString('base64')}`
    } catch {
      crestData = null
    }
  }
  ...
  const composed = new ImageResponse(
    (
      <div
        style={{
          width: '100%',
      ...
      >
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', maxWidth: 720 }}>
            ...
            >
              {total > 0
                ? `${total} centri tecnici attivi, istruttori con nome e cognome.`
          ...
          {crestData ? (
            ...
            <img src={crestData} alt="" width={220} height={220} />
          ) : null}
        </div>
  ...
  return new Response(await composed.arrayBuffer(), {

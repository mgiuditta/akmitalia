 ...
 *  Copiata da prototipo-scala: e' codice usa e getta, e importarlo da un altro
...
export function Switcher({ varianti, corrente }: { varianti: [string, string][]; corrente: string }) {
  ...
  return (
    <div className="switcher">
      <button onClick={() => vai(-1)} aria-label="Variante precedente">
        ←
      </button>
      <span>
        {varianti[i][0].toUpperCase()} · {varianti[i][1]}
      </span>
      <button onClick={() => vai(1)} aria-label="Variante successiva">
        →
      </button>
    </div>
  )

 ...
 *  Copiata da prototipo-home: e' codice usa e getta, e importarlo da un altro
 *  prototipo legherebbe due rami che vanno cancellati separatamente. */
export function Switcher({
  ...
  return (
    <div className="switcher">
      <button onClick={() => vai(-1)} aria-label="Direzione precedente">
        ←
      </button>
      <span>
        {varianti[i][0].toUpperCase()} · {varianti[i][1]}
      </span>
      <button onClick={() => vai(1)} aria-label="Direzione successiva">
        →
      </button>
    </div>
  )

...
import { MAX_VIDEO_MB } from '@/collections/Media'
...
const item: React.CSSProperties = { marginTop: 'calc(var(--base) / 2)' }
const summary: React.CSSProperties = { cursor: 'pointer', fontWeight: 600 }

export function BackgroundGuide() {
  return (
    <div style={{ marginBottom: 'var(--base)', maxWidth: '72ch', lineHeight: 1.5 }}>
      ...
      <ul style={{ margin: 'calc(var(--base) / 2) 0 0', paddingLeft: '1.2em' }}>
        ...
        <li>
          ...
          {MAX_VIDEO_MB} MB (meglio sotto 8). Niente scritte, loghi, bande o sigle: il titolo
      ...
      <details style={item}>
        <summary style={summary}>Brief da mandare al videomaker</summary>
        <pre style={block}>{BRIEF}</pre>
      </details>

      <details style={item}>
        <summary style={summary}>Esportare il file giusto (ffmpeg)</summary>
        <pre style={block}>{FFMPEG}</pre>
      </details>

      <details style={item}>
        <summary style={summary}>Prompt per un generatore video (Veo, Kling, Runway)</summary>
        <p style={{ margin: 'calc(var(--base) / 2) 0 0' }}>
          Le persone generate non sono istruttori AKM: usalo come segnaposto finche non arriva il
          girato vero.
        </p>
        <pre style={block}>{PROMPT}</pre>
      </details>
    </div>
  )
}

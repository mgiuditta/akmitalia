...
const withoutTail = (p: string) => p.replace(/\/$/, '') || '/'
...
export function Menu({
  ...
  const ctx = useRef<ReturnType<Gsap['context']> | null>(null)
  const pathway = usePathname()
  const onPage = withoutTail(pathway ?? '') === withoutTail(cta.href)
  ...
  useEffect(() => {
    ...
    const warm = () => {
      /* Non `import('gsap')`: quel barile si porta dietro ogni plugin.
         Il perche' del percorso preciso sta in src/gsap.d.ts. */
      void import('gsap/dist/gsap.min.js').then((m) => {
        if (!cancelled) gsapRef.current = m.gsap
      })
    }
  ...
  const choreography = useCallback((opens: boolean, onEnd?: () => void) => {
    const node = root.current
    const gsap = gsapRef.current
    if (!node || !gsap) return false
    if (window.matchMedia(REDUCED_MOTION).matches) return false

    ctx.current?.revert()
    ctx.current = gsap.context(() => {
      const backdrops = gsap.utils.toArray<HTMLElement>('.menu__backdrop')
      const texts = gsap.utils.toArray<HTMLElement>('.menu__text')
      const sideTexts = gsap.utils.toArray<HTMLElement>('.menu__ordinal, .menu__detail')
      const rule = '.menu__rule'
      ...
      if (opens) {
        ...
        tl.to(sideTexts, { autoAlpha: 0, duration: 0.18, ease: 'power2.in' })
          .to(
            texts,
            { yPercent: -120, duration: 0.3, stagger: 0.03, ease: 'power2.in' },
            '<',
          )
          .to(rule, { scaleX: 0, duration: 0.3, ease: 'power2.in' }, '<')
          .to(
            backdrops,
  ...
  useEffect(() => {
    ...
    for (const el of backdrop) if (el) el.inert = open
    document.documentElement.classList.toggle('menu-open', open)

    if (open) {
      node.dataset.menu = 'open'
      choreography(true)
    ...
    if (node.dataset.menu !== 'open') return

    const close = () => {
      node.dataset.menu = 'closed'
    }
    if (!choreography(false, close)) close()
  }, [open, choreography])
  ...
  return (
    <>
      ...
      >
        ...
        <ul className="menu__list" onClick={() => setOpen(false)}>
          {items.map((item, i) => (
            <li className="menu__item" key={item.href}>
              <span className="menu__ordinal" aria-hidden="true">
                {ordinal(i)}
              </span>
              <Link className="menu__link" href={item.href}>
              ...
              {item.detail ? (
                <span className={item.live ? 'menu__detail status' : 'menu__detail'}>
                  {item.detail}
                </span>
              ) : null}
            </li>
      ...
      <Link
        ...
        href={onPage ? '#modulo' : cta.href}
      >
        {onPage ? 'Vai al modulo' : cta.text}
      </Link>

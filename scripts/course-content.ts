...
const items = (list: string[]) => list.map((item) => ({ voce: item }))
...
for (const [slug, data] of Object.entries(contents)) {
  ...
  await payload.update({
    ...
    data: { ...data, generateSlug: false, _status: 'published' } as never,
    overrideAccess: true,
  })

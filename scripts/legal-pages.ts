...
for (const page of [privacy, cookie]) {
  ...
  const data = { ...page, generateSlug: false, _status: 'published' } as never
  ...
  payload.logger.info(`${docs[0] ? 'Aggiornata' : 'Creata'} ${saved.path}`)
}
...
const contacts = await payload.findGlobal({ slug: 'contatti', depth: 0, overrideAccess: true })
if (!contacts.modulo?.paginaPrivacy) {
  const { docs } = await payload.find({
    collection: 'pagine',
  ...
  if (docs[0]) {
    try {
      ...
      await payload.updateGlobal({
        slug: 'contatti',
        data: { ...contacts, modulo: { ...contacts.modulo, paginaPrivacy: docs[0].id } } as never,
        overrideAccess: true,
      })

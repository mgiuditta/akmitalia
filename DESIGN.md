...
Il repertorio è chiuso e sono cinque voci: l'entrata scandita di `.rivela` (scroll-driven in CSS, nessun observer), la sequenza a tempo dell'eroe, l'apertura dei `details` del bivio (`interpolate-size` più `::details-content`, miglioramento progressivo senza JavaScript), l'ingresso in cascata dei marker di mappa, e **l'apertura del pannello del menu**: i tre fondi sfalsati scandiscono l'arrivo del foglio scambiando il valore della superficie, ed è l'unica voce affidata a GSAP (`docs/adr/0006`). Niente pin, niente scroll-hijack, niente GSAP fuori dal pannello del menu. Se un'animazione non si spiega in una frase — gerarchia, sequenza di lettura, riscontro a un gesto, cambio di stato — non entra: per questo i titoli del menu entrano traslando e non più ruotati di sei gradi, che era l'unica parte che non si spiegava.

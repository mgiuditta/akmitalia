...
**La Regola del Colore Semantico.** Esistono due colori e due soli ruoli, dichiarati in `docs/adr/0005`. **Rosso `#E30917` = azione**: superficie del solo `button-primary`, etichetta bianca (4.86:1), `#B00711` per hover e active (7.28:1). Non dice mai «selezionato» e non dice mai «attento»: un filtro attivo si distingue scambiando il valore della superficie, un errore si dichiara a parole e con un segno disegnato, in inchiostro. **Verde `#00B44B` = presenza**: solo segnale di dato vivo (centro attivo, marker di mappa, conteggio dei centri), e su fondo chiaro passa a `#006B2C` perché il verde chiaro su bianco si ferma a 2.75:1. Nessun terzo ruolo, nessuna terza tinta.

**Il verde sta dove distingue, non dove si ripete.** Un segno di presenza uguale su ogni riga di un elenco non è un dato vivo, è una decorazione: l'elenco di `/centri` contiene per costruzione i soli centri attivi, quindi lì il verde non c'è e si dichiara semmai l'eccezione, cioè un centro non attivo. Resta dove separa davvero: la scheda di un centro, che si apre anche per un centro chiuso; il conteggio dei centri che tengono un percorso, che cambia riga per riga; il marker della mappa. E uno zero non è un dato vivo: la riga sparisce invece di stampare «0 attivi» con il quadrato verde (`docs/adr/0013`).
...
Il repertorio è chiuso e sono sei voci: l'entrata scandita di `.reveal` (scroll-driven in CSS, nessun observer), la sequenza a tempo dell'eroe, l'apertura dei `details` del bivio (`interpolate-size` più `::details-content`, miglioramento progressivo senza JavaScript), l'ingresso in cascata dei marker di mappa, e **l'apertura del pannello del menu**: i tre fondi sfalsati scandiscono l'arrivo del foglio scambiando il valore della superficie, ed è l'unica voce affidata a GSAP (`docs/adr/0006`). La sesta è il video di sfondo dell'eroe, quando il cliente lo carica: monocromo come la foto che sostituisce, lento, fermabile, assente sotto `prefers-reduced-motion` (`docs/adr/0015`). Niente pin, niente scroll-hijack, niente GSAP fuori dal pannello del menu. Se un'animazione non si spiega in una frase — gerarchia, sequenza di lettura, riscontro a un gesto, cambio di stato — non entra: per questo i titoli del menu entrano traslando e non più ruotati di sei gradi, che era l'unica parte che non si spiegava.

`.reveal` scandisce **un elenco**: righe del bivio, punti della prima volta, schede dei centri, voci dell'albo. Non sta sulla prosa — le sezioni di un'informativa non sono un elenco — e il suo `animation-range` si chiude a `cover 18%`: più in là il blocco raggiunge l'opacità piena a due terzi di schermo e il testo in basso si legge velato.

**La Regola del Cromo Fermo.** Il cromo strutturale non si anima. Oltre al motivo estetico ce n'è uno meccanico: un `transform` su `.header` o sulla sua griglia le rende blocco contenitore di ogni figlio `position: fixed`, e il pannello del menu collasserebbe dentro i 77px della barra. Si anima il pannello e i suoi figli, mai ciò che li contiene.

## 6. Components

- **`navbar`** — barra fissa, **77px su ogni breakpoint**, fondo `#000000`, testo `#FFFFFF`. Nessun bordo e nessun blur, opaca a ogni posizione di scroll, chiusa in fondo dal filetto tricolore da 3px, che sta **dentro** l'altezza. Sopra i 1024px in riga stanno marchio, voci in `nav-link` e bottone CTA; sotto stanno marchio, CTA e bottone del menu, e le voci vivono nel componente `menu`. Due presentazioni, un solo landmark esposto per volta. Voci e CTA vengono dal global `Navigazione`. La CTA non si nasconde mai, perché la richiesta di contatto è l'unico esito misurabile del sito; sulla pagina che indica non sparisce ma cambia parola e destinazione («Vai al modulo», l'ancora del modulo), perché un bottone che ripete parola per parola l'H1 sotto e porta dove si è già non è un invito. Vedi `docs/adr/0007` e `docs/adr/0008`.
...
- **`button-secondary`** — fondo `#1C1C1C`, testo `#FFFFFF`, stesso padding e stessa tipografia del primario, ma piatto. Per azioni a minore enfasi, dove uno scuro-su-scuro deve arretrare. Sono secondari i due inviti dell'eroe: orientano, non convertono, e nella prima schermata il rosso resta uno solo, quello della barra.
...
- **`map__mark`** — il marker della mappa. Nodo da 44×44, quasi tutto trasparente: è il bersaglio minimo da dito, non la misura del segno. Il segno visivo è una targhetta da 14px in verde presenza con anello chiaro da 2px su un'asta da 13px, in fondo al nodo, dove cade l'ancora: la punta sta sulle coordinate e l'aria sopra si prende il dito senza spostare niente sulla carta. Raggio 0 come tutto il resto: nessun pin tondo. Raggiungibile da tastiera, apre il proprio popup con invio. Ha un secondo stato, `map__mark--nearest`: targhetta da 20px e nome del comune scritto accanto, perché il valore non identifica mai da solo (Regola dell'Etichetta). I marker entrano in sequenza con un ritardo a cascata, e l'animazione tocca **solo l'opacità**: su un marker `transform` è la posizione sulla mappa.
- **`pathway__row`** — la riga dell'indice dei percorsi: fascia a tutta larghezza sul ruolo di superficie del corso, ordinale, domanda in `display-md`, nome scritto, sommario, e in coda i destinatari e quanti centri lo tengono. È un link intero. L'hover scambia il valore di superficie, mai la tinta. Il marchio del percorso è inchiostro su trasparente e sulle superfici scure si inverte, non si nasconde.
- **`course__trial`** — la riga che dimostra un percorso, da sola su nero. L'enfasi la fanno l'isolamento e il valore della superficie, non la scala: resta in `heading-lg` (Roboto 700, 22px), perché la Regola dello Stacco Netto non ammette Roboto sopra i 22px e trecento caratteri in Anton maiuscolo non si leggono.
- **`numbered`** — l'elenco dei risultati di un percorso: due colonne sopra gli 800px, ordinale in Anton, un filetto grigio per riga. È la forma che un elenco lungo prende quando ogni voce merita di essere letta.
- **`labels`** — le voci brevi di «su cosa si lavora»: righe corte in fila, un contorno da 1px e nessun riempimento. Non sono bottoni e non sono stati: non prendono né superficie né colore.
...
- **Do** dare 44px di bersaglio a ogni controllo isolato: link del piede, voci in riga della barra, briciole, filtri, cambio mese, zoom della mappa, marker. Un link dentro una frase non è un controllo isolato e resta della misura del testo.
- **Do** far seguire all'anello di fuoco il valore della superficie (`--ring`), non il colore dell'elemento: su un bottone rosso l'etichetta è bianca, e un anello bianco a 3px di offset cade sul bianco della sezione.
